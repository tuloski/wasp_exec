...
void Exec::Leashing::callbackAckMission(const mms_msgs::Ack_mission::ConstPtr& msg){ //don't think leashing will have ack, but just TODO check for seq with low level
...
void Exec::Leashing::start () {
    ...
    try {
	    ...
	    /*if (!get_param("desired-horizontal-distance", desired_horizontal_distance)) {
			desired_horizontal_distance = 0.0f;
	    }
		if (!get_param("desired-vertical-distance", desired_vertical_distance)) {
			desired_vertical_distance = 2.0f;
	    }*/
